...
impl<'a> Widget for ProcessesPanel<'a> {
    fn render(self, area: Rect, buf: &mut Buffer) {
        let title = if self.filter_text.is_empty() { " Processes ".to_string() }
        else { format!(" Processes [filter: {}] ", self.filter_text) };
        ...
        let rows: Vec<Row> = self.processes.iter().enumerate().map(|(i, p)| {
            ...
            Row::new(vec![p.pid.to_string(), p.name.clone(), format!("{:.1}", p.cpu_percent),
                format_bytes(p.memory_bytes), ports, p.command.chars().take(30).collect::<String>()]).style(style)
        }).collect();
...
mod tests {
    ...
    #[test] fn test_render_no_panic() {
        let procs = vec![ProcessInfo { pid: 1234, name: "node".into(), command: "node server.js".into(),
            user: "yuji".into(), cpu_percent: 12.5, memory_bytes: 340_000_000, threads: 8,
            parent_pid: Some(1), listening_ports: vec![3000], start_time: 0 }];
        let p = ProcessesPanel { processes: &procs, selected: 0, filter_text: "", is_focused: true, tree_mode: false };
