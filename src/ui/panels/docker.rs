...
impl<'a> Widget for DockerPanel<'a> {
    fn render(self, area: Rect, buf: &mut Buffer) {
        let title = if self.filter_text.is_empty() { " Docker ".to_string() }
        else { format!(" Docker [filter: {}] ", self.filter_text) };
        ...
        if !self.is_available {
            ...
            if inner.width > 18 && inner.height > 0 {
                buf.set_string(inner.x, inner.y, "Docker not detected", Style::default().fg(Color::DarkGray));
            }
            return;
        }
        ...
        let rows: Vec<Row> = self.containers.iter().enumerate().map(|(i, c)| {
            ...
            let ports_str = c.ports.iter().map(|p| format!("{}:{}", p.host, p.container)).collect::<Vec<_>>().join(", ");
            Row::new(vec![c.name.clone(), c.image.clone(), c.state.as_str(),
                format!("{:.1}", c.cpu_percent), format_bytes(c.memory_bytes), ports_str]).style(style)
        }).collect();
...
mod tests {
    ...
    #[test] fn test_render_no_panic() {
        let c = vec![ContainerInfo { id: "abc".into(), name: "app-web".into(), image: "node:18".into(),
            state: ContainerState::Running, cpu_percent: 12.0, memory_bytes: 340_000_000, memory_limit: 1_000_000_000,
            ports: vec![PortMapping { host: 3000, container: 3000, protocol: "tcp".into() }],
            compose_project: Some("myapp".into()), created: "2026-04-03".into() }];
        let p = DockerPanel { containers: &c, selected: 0, filter_text: "", is_focused: true, is_available: true };
        let mut buf = Buffer::empty(Rect::new(0, 0, 60, 10));
        p.render(Rect::new(0, 0, 60, 10), &mut buf);
    }
    #[test] fn test_unavailable() {
        let p = DockerPanel { containers: &[], selected: 0, filter_text: "", is_focused: false, is_available: false };
        let mut buf = Buffer::empty(Rect::new(0, 0, 60, 10));
        p.render(Rect::new(0, 0, 60, 10), &mut buf);
    }
}
