...
impl<'a> Widget for PortsPanel<'a> {
    fn render(self, area: Rect, buf: &mut Buffer) {
        let title = if self.filter_text.is_empty() { " Ports ".to_string() }
        else { format!(" Ports [filter: {}] ", self.filter_text) };
        ...
        let rows: Vec<Row> = self.entries.iter().enumerate().map(|(i, e)| {
            ...
            Row::new(vec![
                ...
                e.process_name.clone(), e.pid.to_string(),
                format!("{:.1}", e.cpu_percent), format_bytes(e.memory_bytes),
            ]).style(style)
        }).collect();
        let table = Table::new(rows, [
            Constraint::Length(7), Constraint::Length(5), Constraint::Min(10),
            Constraint::Length(7), Constraint::Length(6), Constraint::Length(8),
...
mod tests {
    ...
    fn sample() -> Vec<PortEntry> {
        vec![PortEntry { port: 3000, protocol: Protocol::Tcp, address: "127.0.0.1".into(),
    ...
    #[test] fn test_render_no_panic() {
        let e = sample();
        let p = PortsPanel { entries: &e, selected: 0, filter_text: "", is_focused: true };
        let area = Rect::new(0, 0, 60, 10);
        let mut buf = Buffer::empty(area);
        p.render(area, &mut buf);
    }
    #[test] fn test_render_with_filter() {
        let e = sample();
        let p = PortsPanel { entries: &e, selected: 0, filter_text: "node", is_focused: true };
        let area = Rect::new(0, 0, 60, 10);
        let mut buf = Buffer::empty(area);
        p.render(area, &mut buf);
    }
